...
async def rot_n(self, chan, src, msg):
    args = msg.split(" ", 1)
    if len(args) < 2:
        await out.msg(self, "caesar", chan, ["need rot number and message"])
        return
...
async def init(self):
    ...
    self.help["communist"] = [
        "communist - \x038,5 ☭ SEIZE THE MEANS OF CHAOS PRODUCTION ☭\x04"
    ]
    ...
    self.help["cowsay"] = [
        "cow{say,think} [args] - use /bin/cow{say,think} to generate ascii art"
    ]
