...
def get_all_logs(chan, msg):
    ...
    for line in rawlogs:
        ...
        user = data[1]
        mesg = data[2]
        logs.append({"time": time, "user": user, "msg": mesg})
    return logs
...
async def get_stats_by_phrase(self, chan, src, msg, phrases, label):
    ...
    await out.msg(self, modname, chan, [f"{label} people at {targetchan}: {output}"])
...
async def noisiest(self, chan, src, msg):
    ...
    for i in sorted(stats.items(), key=lambda i: i[1], reverse=True):
        ...
        output += "{} ({}%, {} msgs), ".format(nohighlight(i[0]), percentage, i[1])
        ctr += 1
    output = output[:-2]  # trim ', '
    await out.msg(self, modname, chan, [f"top talkers at {targetchan}: {output}"])
...
async def init(self):
    ...
    self.help["usrstats"] = [
        "usrstats - display statistics on various users",
        "usrstats subcommands: noisiest happiest saddest angriest",
    ]
    self.help["usrstats noisiest"] = [
        "usrstats noisiest [chan] - get the top talkers for [chan] (default: current)"
    ]
    self.help["usrstats happiest"] = [
        'usrstats happiest [chan] - get the users who type "lol", "lmao", ":)", ":D", etc in their messages the most on [chan]. (default: current)'
    ]
    self.help["usrstats saddest"] = [
        'usrstats saddest [chan] - get the users who type ":/", ":(", etc in their messages the most on [chan]. (default: current)'
    ]
    self.help["usrstats angriest"] = [
        "usrstats angriest [chan] - get the users who curse the most on [chan]. (default: current)"
    ]
